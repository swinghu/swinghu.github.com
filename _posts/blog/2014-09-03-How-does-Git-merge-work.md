...
如果你仔细，很有观察力，你可能已经发现我在上述说明中存在的一个漏洞了：由于`commit` \\(A\\)和\\(B\\)可能各自又包含`commit`,他们最近的共同祖先可能不是唯一的！一般，他们最有可能的情形是，最近的共同祖先是\\(C_{1},C_{2},C_{3},C_{4},\cdots C_{k-1} C_{k}\\)
 ，在这种情况下，`git merge` 操作将会递归的执行：它首先构造合并\\(C=C_{1}\vee C_{2} \vee C_{3} \cdots C_{k-1}\vee C_{k}\\)
 ，并以此作为三向合并
$$
\begin{bmatrix} \
A \
